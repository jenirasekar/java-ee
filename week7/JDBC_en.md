...
- ------
  ...
  When we insert a record into the database, the primary key is often auto-generated by the database, for example using auto-increment.

  At this moment, although we can see the new ID directly in the database table, in our Java program we usually only get the number of affected rows, like “1 row inserted”. We don’t automatically know what the new primary key value is.

  So what should we do if we need this ID?

  In this case, we can use `getGeneratedKeys()` in JDBC to retrieve the auto-generated primary key after the insert operation. Then we can assign this value back to our Java object.

  For example, if we insert an employee, after the insert we can get the generated `emp_id`, and then call `emp.setEmpId(id)` to complete the backfill.

  This process is called primary key backfill, and it is very important when we need to continue using this object in later operations, such as updates or associations with other tables.
...
>
> 3. `ThreadLocal.remove()`: remove the value of the current thread's shared variabl
>
>    So the question is, why do we need `ThreadLocal`?
>
>    In a multithreaded environment, if multiple threads share the same variable or resource, such as a database connection, it can easily lead to data confusion and thread safety problems. For example, one thread may accidentally modify or close a connection that another thread is still using.
>
>    To solve this problem, we can give each thread its own independent copy of the variable. That’s exactly what `ThreadLocal` does.
>
>    It does not really make the variable “shared”. Instead, it creates a separate copy of the variable for each thread. So even though the code uses the same `ThreadLocal` object, each thread operates on its own data, and they do not interfere with each other.
>
>    This is especially useful in scenarios like database connections. Each thread can have its own connection stored in `ThreadLocal`, so we don’t need to pass the connection object through multiple layers of methods, and at the same time, we avoid concurrency issues.
>
>    So in short, `ThreadLocal` helps us achieve two things:
>     first, **data isolation between threads**, and second, **simplifying code by avoiding repeated parameter passing**.
